	...
	keys = {
		{
			"<leader>fe",
			function()
				-- Try to get the Git root directory using the command
				local git_root = vim.fn.systemlist("git rev-parse --show-toplevel")[1]
				if vim.v.shell_error == 0 and git_root and git_root ~= "" then
					-- If the Git root directory is found, use it
					require("neo-tree.command").execute({ toggle = true, dir = git_root })
				else
			...
			desc = "Explorer NeoTree (Git Root, README Dir, or CWD)",
		},
		{
			"<leader>fE",
			function()
